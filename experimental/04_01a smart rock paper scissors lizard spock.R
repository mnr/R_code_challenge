...
# userChoices is our record of choices made
userChoices <- NULL
userChoices[rpslsNames] <- .0001

while (TRUE) {
  userChoiceProb <- userChoices / sum(userChoices)
  
  computerChoice <- rpslsNames[which.max(userChoiceProb)]
  
  userChoice <-
    rpslsNames[menu(rpslsNames, graphics = TRUE, title = "What do you choose? ")]
  
  if (length(userChoice) == 0) break # user cancels the game
  
  # now the computer remembers the user choice
  userChoices[userChoice] <- userChoices[userChoice] + 1
